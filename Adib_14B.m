...
date_start=[2012,01,01];           %Insert custom start and end dates
date_end=  [2012,12,31];           %Period spanning through 3 consecutive years is the maximum
...
%Frequency band
f_1=input('Frequency band? ');
f_2=f_1;
f_array=unique([f_1,f_2]);

%Nighttime setup
disp('Local night time? ')
LT_choice={'22 - 00','00 - 02','02 - 04','04 - 06'};
for i=1:4
    disp(sprintf('%d. %s',i,LT_choice{i}));
end
LT_chosenID=input('Enter ID number ');
LT_chosen=LT_choice{LT_chosenID};

LT_start=str2num(LT_chosen(1:2));
LT_end=str2num(LT_chosen(6:7));
%% Time period
...
for i=1:days_num
    [H_spec,F_spec,T_spec]=spectrogram(H_night(:,i),'yaxis',hamming(n_win),n_ovrlp,n_fft,1);
    [D_spec,F_spec,T_spec]=spectrogram(D_night(:,i),'yaxis',hamming(n_win),n_ovrlp,n_fft,1);
    [Z_spec,F_spec,T_spec]=spectrogram(Z_night(:,i),'yaxis',hamming(n_win),n_ovrlp,n_fft,1);
    [G_spec,F_spec,T_spec]=spectrogram(G_night(:,i),'yaxis',hamming(n_win),n_ovrlp,n_fft,1);
...
EQ_det1=find(0.025*EQ_sel(:,2)<EQ_sel(:,3)-4.5);
EQ_det2=find(EQ_sel(:,3)>=6.5);
EQ_det3=unique(vertcat(EQ_det1,EQ_det2));
EQ_det4=EQ_sel(EQ_det3,:);

disp(sprintf('Found %d detectable earthquake(s) in the whole period..',size(EQ_det4,1)));
if size(EQ_det4,1)>1
    for i=1:size(EQ_det4,1)
       disp(sprintf('%d. %s',i,datetime(EQ_det4(i),'ConvertFrom','datenum','Format','dd/MM/yyyy HH:mm'))); 
    end
    chooseEQ=input('Select an earthquake by inputting its ID number ');
    DOI_0=EQ_det4(chooseEQ,1);
elseif size(EQ_det4,1)==1
    DOI_0=EQ_det4(1,1);
else
    DOI_0=[];
end
...
bar(days_vec,anom,'r');
bar(days_vec,dstb,'g');
if  ~any(isnan(EQ_sel(1,:))) && ~isempty(DOI_0)
    for i=1:5
        if i==1 
...
elseif asksave==1
    for i=1:100
        if exist(strcat(path2,figname1,sprintf('-%d',i),'.tif'))~=2
            ...
            disp('Figure saved in the following directory: ');
            disp(sprintf('%s',strcat(path2,figname1ext,'.tif')));
            break
        end
    end
end    
