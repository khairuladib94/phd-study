stn1='MUT';                           %Abbreviation of station name
station1='Mutinlupa';                %Station full name
stn2='DAV';
station2='Davao';
...
ZH_mup3sig=ZH_mu+3*ZH_sig;
ZD_mup3sig=ZD_mu+3*ZD_sig;
ZG_mup3sig=ZG_mu+3*ZG_sig;

ZH_mum3sig=ZH_mu-3*ZH_sig;
ZD_mum3sig=ZD_mu-3*ZD_sig;
ZG_mum3sig=ZG_mu-3*ZG_sig;
...
title(lgd,'Near to:');
title(sprintf('Polarisation of diurnal variation ratio | %d | %s-%s station (%.0f km)',year,station1,station2,stn_dis));
hold off

subplot(5,1,2);
plot(days,ZH,days,ones(1,days_num)*ZH_mup3sig,'r--',days,ones(1,days_num)*ZH_mum3sig,'r--');
xlim([1 days_num]);
ylabel('Z/H');

subplot(5,1,3);
plot(days,ZD,days,ones(1,days_num)*ZD_mup3sig,'r--',days,ones(1,days_num)*ZD_mum3sig,'r--');
xlim([1 days_num]);
ylabel('Z/D');

subplot(5,1,4);
plot(days,ZG,days,ones(1,days_num)*ZG_mup3sig,'r--',days,ones(1,days_num)*ZG_mum3sig,'r--');
xlim([1 days_num]);
ylabel('ZG');
