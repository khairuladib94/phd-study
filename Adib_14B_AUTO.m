%% Purpose
%Same like Adib_14B but runs automatically
...
today=strcat(char(datetime('today','Format','dd-MM-yyyy')),'\Adib_14B_AUTO\');
...
for precursor=2:size(readxcel,1)
    ...
    date_pre=datetime(readxcel{precursor,15},'InputFormat','dd-MMM-yy','Format','dd/MM/yyyy');
    idx_pre=find(days_vec==date_pre);
    text(days_vec(idx_pre),ZG(idx_pre),'*','HorizontalAlignment','center','VerticalAlignment','top','FontSize',20,'Color',[1 1 1]);
    ...
    output_pre{1,1}=round(ZG_mu,4);
    output_pre{1,2}=round(mean(ZG_mup2sig),4);
    output_pre{1,3}=round(readxcel{precursor,17}-mean(ZG_mup2sig),4);
    output_pre{1,4}=round(readxcel{precursor,17}/mean(ZG_mup2sig),4);
    xlsrange=strcat('AA',string(precursor),':AD',string(precursor));
    xlswrite(xlsfilename,output_pre,xlssheetname,xlsrange);
