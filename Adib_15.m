...
% azimuthal angles accuracy by scanning 4 night time periods and all
% frequencies in ULF
...
for stn_num=4%1:size(stn_vec,1)
    for year=2005:2018
        ...
        for l=1:4
            for i=1:days_num
                ...
                H_nightseg=reshape(H_night(:,i,l),n_win,[]);
                D_nightseg=reshape(D_night(:,i,l),n_win,[]);
                Z_nightseg=reshape(Z_night(:,i,l),n_win,[]);
                G_nightseg=reshape(G_night(:,i,l),n_win,[]);
        ...
        %% Clearing variables
        clearvars -except mag_min dis_max depth_max LT_start LT_end stn_num year stn_vec stn_MAGDAS station_vec index_geomag EQ_WORLD
        pause(0.5);
        clc;
    end
end
...
% azimtheta=round(azim_theta,0);
% azimtheta=reshape(azimtheta,[],1);
% for i=1:361
%     occur(i)=sum(azimtheta==i-1);
% end
% scatter(0:360,occur,'r');
% 
